...
public class fejesjoco_SE1
{
    // algorithm settings, feel free to mess with it
    const bool AVERAGE = false;
    const int NUMCOLORS = 36; //32;
    const int WIDTH = 216;//256;
    const int HEIGHT = 216;//128;
    const int STARTX = 216/2;//128;
    const int STARTY = 216/2;//64;
    ...
    public static Bitmap CreateImage()
    {
        ...
        colors = colors.OrderBy(_ => rnd.Next(3) - 1).ToList();
        ...
        for (var i = 0; i < colors.Count; i++)
        {
            ...
            //         }
            //     }
            //     img.Save("result" + chkidx + ".png", ImageFormat.Png);
            // }
        }
